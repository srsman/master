...
(function () {
    ...
    class Apis {
        ...
        static async fetchJavStore(code) {
            ...
            res = res.querySelector(".news a img[alt*='.th']")?.src;
            if (!res?.startsWith("https")) return re;
        ...
        static async fetchJavSpyl(code) {
            ...
            let res = await request(`https://api.javspyl.tk/api/?${code}`, {}, "GET", {
                headers: { origin: "https://javspyl.tk" },
                responseType: "text",
            });
            if (!res?.msg || res?.code) return re;

            res = res.msg;
            if (res.trim() === "no" || res.endsWith(".m3u8")) return re;
